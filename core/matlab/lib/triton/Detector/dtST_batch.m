function dtST_batch(BaseDir, DataFiles, Labels, mode, params)
...
% $Id: dtST_batch.m,v 1.28 2009/08/03 17:39:15 mroch Exp $
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             + 3;      % add in the other side that matlab doesn't do
    ...
    for s = 1:max(WhistlePos, ClickPos)
        ...
        ioWriteLabel(fullfile(BaseDir, OutLabel), detections{s}, Labels{idx}); 
    end
end  
if ~ isempty(ProgressH)
  delete(ProgressH);
