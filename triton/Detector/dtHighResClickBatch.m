function dtHighResClickBatch(DataFiles, LabelFiles, MLFname, SCPname, varargin)
% dtHighResClickBatch(DataFiles, LabelFiles, MLFname, SCPname, optional arguments)
% Given a list of data and label files,
% run the short term detection algorithms on each DataFile within
% the ranges specified by LabelFiles.
%
% MLFname is the name of the master label file (HTK) that will be
% written.  Similary, SCPname lists the files that HTK will need
% to process.
%
% FeatureExt is the desired extension for the resulting feature file
% Only detections whose label contains the substring LabelFilter
% will be used.
%
% Optional arguments:
...
% $Id: dtHighResClickBatch.m,v 1.51 2010/02/25 22:10:26 mroch Exp $
...
% Start main function --------------------------------------------
  ...
  CutPeakBelow_Hz = 20000; % discard click if peak frequency below X 
  CutPeakAbove_Hz = 70000; % discard click if peak frequenc above X
  ...
  % initialize wavelet stuff if needed
  if strcmp(options.type, 'wvt')
    QuadMirrorFilt = MakeONFilter('Symmlet', 10);
    SplitLevel = 4;
  end

  if length(DataFiles) ~= length(LabelFiles)
  ...
  for idx=1:N; % for each data file
    ...
    if FileTypes(idx) == 1                % wav file
      if isempty(DateRE)
        disp_mesg(sprintf('%s - no pattern to parse date - skipping', ...
                          DataFiles{idx})); 
        continue
      else
        hdr.start.dnum = dateregexp(DataFiles{idx}, DateRE, ...
                                    DefaultDate, OffsetFromDate);
        if hdr.start.dnum == DefaultDate
          disp_mesg(sprintf('%s - cannot parse date', DataFiles{idx}));
        end
      end
    end
    ...
    if hdr.fs ~= PreviousFs
      ...
      % Wavelet transforms need a power of 2
      PowerOfTwo = strcmp(options.type, 'wvt');
      if PowerOfTwo
        % Ensure that framing is a power of two
        FrameLength_pow2 = 2^ceil(log2(hdr.fs * FrameLength_us / us_per_s));
        if FrameLength_pow2 ~= FrameLength_samples
          disp_msg(sprintf('Adjusting frame from %f us (%d samples)', ...
                 FrameLength_us, FrameLength_samples));
          FrameLength_samples = FrameLength_pow2;
          FrameLength_us = FrameLength_samples / hdr.fs * us_per_s;
        end
      end
      ...
      if ~ isempty(Clicks)
        ...
        for c = find(ValidClicks)
            ...
            [bandwidth_kHz, low_freq, high_freq] = ...
                spNdB_Bandwidth(stMA(signal, 21, -11), NdB, binWidth_kHz);
