function cltrebic(SourceData, PriorData, varargin)
% modified on bictestswb2prior(SourceData, OptionalArguments)
% Evaluate the EB_BIC on a corpus using prior computed from 
% Switch Board 2
%
% Optional arguments
...
switch runenv
 case 'dusky'
      if noEnd
          ...
          % separated channels
          melcepPathChanSep = '/cache/cheng/swb2/';
      else
      ...
      windowSpacePath = '/zal/cheng/matlab/audio/tracking/try/';
      ChangeTimePath = '/zal/cheng/matlab/audio/tracking/changetime/';
      if Display
         outPath = '/zal/cheng/matlab/audio/tracking/odessy/';
      else
         outPath = '/zal/cheng/matlab/audio/tracking/odessy/';   
      end
...
%output to file
if noEnd
    outFile= strcat(outPath,'odessy', '_', ...
                    num2str(WindowLengthMS/1000), '_',...
                    num2str(WindowAdvanceMS/1000),'_',...
                    num2str(EnergyPct), Bayesian, num2str(SegmentLength/1000),'_ne.txt');
else
    outFile= strcat(outPath,'odessy', '_', ...
                    num2str(WindowLengthMS/1000), '_',...
                    num2str(WindowAdvanceMS/1000),'_',...
                    num2str(EnergyPct), Bayesian,num2str(SegmentLength/1000), '.txt');
end
out = fopen(outFile,'w');
...
for foldindx = 1  %:NFold
  [TrainIndices, TestIndices] = trkNfold(foldindx, NFold, UtteranceCount); % TrainIndices not used

  if firstTime 
      tic;
      Prior = trkEstimatePriorsNfolderSwb2(melcepPathChanSep, PriorData, 0.10);                        
  ...
  for idx = 1   %  TestIndices  TEST only one now   %yc
...
%	        bic(end+1) = trkBIC_nFold(segment, winoffset, EnergyPct);
    ...
    %          chgptold =[];
    PeakDetector =  'regression'; % 'simple';
    switch PeakDetector
     case 'simple'
  ...
  %recompute the ebic with clustering %yc
  %pretime = HitTimes{idx};
  % List of all possible change points padded by beginning & ending observation
  pretime = chgpt;  

  retestpt =  [1, round(pretime.*1000./cepSpcMS), cepLen];  %convert to index
  
  rebic = [];
  for i=2:length(retestpt)-1
          segment = data(retestpt(i-1):retestpt(i+1), :);
          winoffset = retestpt(i) - retestpt(i-1);
          rebic(end+1) = trkEB_BIC_nFold(segment, winoffset, ...
                  EnergyPct, Prior, Bayesian);
  end
%  for i = 1:(length(retestpt)-2)    
%
%       segment = data(retestpt(i):retestpt(i+2), :); 
%       %winoffset = round((tail-head)/2);
%       winoffset = retestpt(i+1)-retestpt(i);
%       %case 'eBic'
%       rebic(end+1) = trkEB_BIC_nFold(segment, winoffset, ...
% 				     EnergyPct, Prior, Bayesian);
%  end 
 keyboard
 plot(rebic, retestpt(1:length(retestpt)-2));
