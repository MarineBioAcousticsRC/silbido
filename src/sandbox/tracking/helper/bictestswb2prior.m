function bictestswb2prior(SourceData, PriorData, varargin)
% bictestswb2prior(SourceData, OptionalArguments)
...
switch runenv
 case 'fruitbat'
  ...
  if noEnd
    ...
    % separated channels
    melcepPathChanSep = '/lab/speech/corpora/swb2/cep/'; 
  else
    melcepPath = '/zal/mroch/cheng/MelBand/';
    melcepPathChanSep = '/zal/mroch/cheng/MelBandAllChannel/'; 
  end
  %server path no need to update
  ChangeTimePath = ['/lab/speech/corpora/spidre/changepoint/'];

 otherwise
      % Assume running on 'dusky'
      if noEnd
...
for foldindx = 1:NFold
  ...
  if firstTime && isempty(Prior)
      tic;
      Prior = trkEstimatePriorsNfolderSwb2(melcepPathChanSep, ... 
                PriorData, 0.10);                        
