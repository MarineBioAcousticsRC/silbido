...
%		'eBic' - empirical Bayes BIC - indepence assumption
%	'Bayesian', String
...
switch runenv
 case 'dusky'
      ...
      if Display
         outPath = '/zal/cheng/matlab/audio/tracking/results24/temp/';
      else
      ...
      if SegmentLength == -1
          ...
          outPath = '/zal/cheng/matlab/audio/tracking/results24/6/';
      end  
  ...
  if strcmp(Method, 'eBic')
    if isempty(Priors)
          ...
          if SegmentLength == -1
                ...
                Prior = trkPriorEstimationSeg(melcepPathChanSep, ... 
					        SourceData(TrainIndices), 0.10, SegmentLength);
          end
    else
...
%    fprintf('%s\n', FileName);
    ...
    %          chgptold =[];
    PeakDetector =  'regression'; %'regression'; 
    switch PeakDetector
     case 'simple'
      chgpt = trkPeakTime(fBic, time); %trkPeakTime(fBic, time);
     case 'regression'
      [peaks, Signal] = spPeakSelector(fBic, 'RegressionOrder', 2, ...
				  'Display', 1);
      chgpt = time(peaks);
     otherwise
