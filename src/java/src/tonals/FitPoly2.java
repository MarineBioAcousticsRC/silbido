 ...
 */
public class FitPoly2 implements FitPoly {
	 ...
	 */
	private double std_dev; // standard deviation of predicted variable
	 ...
	 */
	public FitPoly2(int order, double [] x, double [] y, double [] sigma_x, double [] sigma_y, int num_points) {
		doFit(order, x, y, sigma_x, sigma_y, num_points);
	}
	
	
	public FitPoly2(int order, double [] x, double [] y) {
		doFit(order, x_tmp, y_tmp, (double[]) null, (double[]) null, x.length);
	}
	 ...
	 */
	public FitPoly2(int order, Vector<Double> x, Vector<Double> y) {
...
//			Arrays.fill(x_tmp,0.0);
//			Arrays.fill(y_tmp,0.0);
		}
		...
		doFit(order, x_tmp, y_tmp, (double[]) null, (double[]) null, n);
	}
	 ...
	 * @param tone
	 * @param skip_n
	 * @param fit_dphase true - polynomial fit of first difference of phase to frequency
	 *                   false  - polynomial fit of frequency to time.
	 * @param incoming_edge
	 */
	public FitPoly2(int order, tonal tone, int skip_n, boolean fit_dphase, boolean incoming_edge) {
...
//			Arrays.fill(y_tmp,0.0);
		...
		// FIXME the basic problem here is that we created arrays with a size
		// that is probably bigger than the number of nodes we have.  First
		// we can skip some nodes.  Second, we stop adding nodes after an elapsed
		// amount of time.
		
		// build the polynomial
		doFit(order, x_tmp, y_tmp, (double[]) null, (double[]) null, Math.min(i, n));
	}
	...
	/**
	 * Use the Least Squares fit method for fitting a polynomial to 2-D data for
	 * measurements y[i] vs. dependent variable x[i]. This fit assumes there are
	 * errors only on the y measurements as given by the sigma_y array.<br>
	 * <br>
	 * 
	 ...
	 * "A Java Matrix Package" classes. See
	 ...
	 *            - dependent variable
	 * @param sigma_x
	 *            - std. dev. error on each x value (null okay)
	 * @param sigma_y
	 *            - std. dev. error on each y value (null okay)
	 * @param num_points
	 *            - number of points to fit. Less than or equal to the dimension
	 *            of the data arrays
	 */
	private void doFit(int order, double[] x, double[] y, double[] sigma_x,
		...
		if (num_points == 1) {
			...
			linearFit(order, x, y, sigma_x, sigma_y, num_points, nk);
		} else {
			higherOrderFit2(order, x, y, sigma_x, sigma_y, num_points, nk);
		}
	...
	private void linearFit(int order, double[] x, double[] y, double[] sigma_x, double[] sigma_y, int num_points, int nk) {
	...
	private void higherOrderFit(int order, double[] x, double[] y, double[] sigma_x, double[] sigma_y, int num_points, int nk) {
	...
	private void higherOrderFit2(int degree, double[] x, double[] y, double[] sigma_x, double[] sigma_y, int num_points, int nk) {
//		this.degree = degree;
        int N = x.length;
