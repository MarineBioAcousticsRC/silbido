...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function init_ground_truth_tonal
        ...
        while detectedIt.hasNext()
            detected_tonal = detectedIt.next();
            % new copy of a tonal
            detected_tonal_copy = detected_tonal.clone();
            
            % Create ground truth tonal vector
            ground_truth_tonals = ...
                [ground_truth_tonals detected_tonal_copy];
        end
    end
    ...
    function plotSpec_detected_tonal
        ...
        if (DATA.threshold == true)
            [HANDLES.axisH HANDLES.imageH] = dtPlotSpecgram(Filename, blkstart_s,...
                blkstop_s, 'Framing', [Advance_ms, Length_ms], 'Noise', ...
                NoiseMethod, 'Threshold', DATA.thr_energy_dB, 'Range', ...
                [low_cutoff_Hz, DATA.high_cutoff_Hz], 'Render', 'floor');
        else
            [HANDLES.axisH HANDLES.imageH] = dtPlotSpecgram(Filename, blkstart_s,...
                blkstop_s, 'Framing', [Advance_ms, Length_ms], 'Noise',...
                NoiseMethod, 'Range', [low_cutoff_Hz, DATA.high_cutoff_Hz]);
        end
    ...
    function [ntime, nfreq] = getSpline(points, current_point)
        ...
        if length(t) > 1
            ...
            ntime = linspace(t(1),t(end), ...
                (Advance_ms / 1000) * (sampleRate_kHz * scale));
...
% Push button callbacks -------------------------------------
    ...
    function splineFitButton_Callback(hObject, eventdata)
        ...
        while tonal_idx <= length(sel_tonal)
            ...
            pts_n = (Advance_ms / 1000) * (sampleRate_kHz * scale);
            tt = linspace(t(1), t(end), pts_n);
